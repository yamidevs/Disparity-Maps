...
{
    float dist=0.0f,I1=0.0f,I2=0.0f;
    // ------------- TODO -------------
    for(int y = -win;y<= win;y++ ){
        for(int x = -win;x<= win;x++){
           I1 += pow(im1(x + i1 , y + j1) -m1,2);
           I2 += pow(im2(x + i2 , y + j2) -m2,2);
           dist +=  (im1(x + i1 , y + j1) -m1) * (im2(x + i2 , y + j2) -m2);
        }
    }
    return dist / sqrt(I1 *I2 + EPS);
}
...
static void find_seeds(Image<byte> im1, Image<byte> im2,
    ...
    for(int y=win; y+win<maxy; y++) {
        ...
        for(int x=win; x+win<im1.width(); x++) {
            ...
            int bestDisparity = dMin -1;
            while(currentDisparity <= dMax){

                // force the patch to be in the image otherwise just ignore it
                if(win <= x+currentDisparity && x+currentDisparity+win < im2.width()){
                    float currentNCC = ccorrel(im1,x,y,im2,x+currentDisparity,y);
                    // take best NCC
            ...
            //if the best ncc is highter than seed
            if(bestNCC > nccSeed ){
                disp(x,y) = bestDisparity;
                seeds(x,y) = true;
                Q.push(Seed(x,y,bestDisparity,bestNCC));
            }
        }
    }
    std::cout << std::endl;
            ...
            if(0<=x-win && 0<=y-win &&
               ...
               ! seeds(x,y)) {
                ...
                for(int disparity = s.d-1;disparity<=s.d+1;disparity++){
                    // need disparity be between [dmin,dmax]
                    if(x+ disparity +win<im2.width() && y+win<im2.height()
                            && dMin <= disparity && disparity <= dMax
                            && 0<=x+disparity-win && 0<=y-win  ){
                     float currentNCC = ccorrel(im1,x,y,im2,x+disparity,y);
                     if(currentNCC > bestNCC){
                         bestNCC = currentNCC;
...
int main()
{
    ...
    if(crop){
        ...
        I1 = I1.getSubImage(0,0,height,width);
        I2 = I2.getSubImage(0,0,height,width);
    }
